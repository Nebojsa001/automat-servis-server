...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  if (req.user === "superAdmin") {
    //ukoliko je req.user superAdmin moze dodavati rolove
    ...
    //ukoliko nije role je automatski user
    newUser = await User.create({
      firstName: req.body.firstName,
      lastName: req.body.lastName,
      email: req.body.email,
    ...
    //     path.join(templatePath, "paymentInstructions.pug")
    //   );
    //   const renderedTemplate = compiledFunction({ firstName, lastName });

    //   //mailer
    //   const to = req.body.email;
    //   const subject = "Uspešna registracija - Potvrda uplate potrebna";
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.role === "user" && currentUser.isAccessExpired()) {
    return next(
      new appError(
        "Vaš nalog je istekao! Molimo podnesite zahtev za produženje vašeg računa.",
        401
      )
    );
  }
